...
class CustomerFormAPI(http.Controller):
    ...
    def dashboard_data(self, **kwargs):
        ...
        try:
            ...
            _logger.info(f"dashboard_data completed successfully in {duration:.4f} seconds for user: {name}")
            return result
        except Exception as e:
            duration = time.time() - start_time
            _logger.error(f"dashboard_data failed in {duration:.4f} seconds with error: {str(e)}")
            raise
    ...
    def get_all_agencies_web(self, **kwargs):
        ...
        try:
            ...
            duration = time.time() - start_time
            _logger.info(f"get_all_agencies_web completed successfully in {duration:.4f} seconds, returned {len(data)} agencies")
            return result
        except Exception as e:
            duration = time.time() - start_time
            _logger.error(f"get_all_agencies_web failed in {duration:.4f} seconds with error: {str(e)}")
            raise
    ...
    def assign_agency_web(self, pin_lo_id=None, **kwargs):
        ...
        try:
            ...
            if not pin_lo_id:
                duration = time.time() - start_time
                _logger.info(f"assign_agency_web completed in {duration:.4f} seconds with error: Please select an agency.")
            ...
            if not pin_location.exists():
                duration = time.time() - start_time
                _logger.info(f"assign_agency_web completed in {duration:.4f} seconds with error: Selected agency was not found.")
            ...
            duration = time.time() - start_time
            _logger.info(f"assign_agency_web completed successfully in {duration:.4f} seconds for user: {user.name}, agency: {pin_location.location_name}")
            return result
        except Exception as e:
            duration = time.time() - start_time
            _logger.error(f"assign_agency_web failed in {duration:.4f} seconds with error: {str(e)}")
            raise
    ...
    def get_current_agency_web(self, **kwargs):
        ...
        try:
            ...
            if not pin:
                duration = time.time() - start_time
                _logger.info(f"get_current_agency_web completed in {duration:.4f} seconds with error: No current agency assigned.")
                return {'success': False, 'message': 'No current agency assigned.'}
            pin = request.env['pin.location'].sudo().browse(int(pin))
            if not pin.exists():
                duration = time.time() - start_time
                _logger.info(f"get_current_agency_web completed in {duration:.4f} seconds with error: Current agency not found.")
            ...
            duration = time.time() - start_time
            _logger.info(f"get_current_agency_web completed successfully in {duration:.4f} seconds for user: {user.name}")
            return result
        except Exception as e:
            duration = time.time() - start_time
            _logger.error(f"get_current_agency_web failed in {duration:.4f} seconds with error: {str(e)}")
            raise
    ...
    def apiCustomerForm(self, **kwargs):
        ...
        try:
            user = request.env.user
            _logger.info("vinayyyyyy2121232324")
            ...
            duration = time.time() - start_time
            _logger.info(f"apiCustomerForm completed successfully in {duration:.4f} seconds for user: {user.name}, customer_id: {customer.id}")
            return result
        except Exception as e:
            duration = time.time() - start_time
            _logger.error(f"apiCustomerForm failed in {duration:.4f} seconds with error: {str(e)}")
            raise