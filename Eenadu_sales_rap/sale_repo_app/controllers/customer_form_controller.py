...
class CustomerFormAPI(http.Controller):
    ...
    def get_filtered_customer_forms(self, **params):
        ...
        required_fields = [
            'id', 'agent_name', 'agent_login', 'unit_name', 'date', 'time', 'family_head_name',
            ...
            'location_address', 'location_url', 'face_base64', 'for_consider', 'shift_to_EENADU',
            'Willing_to_Shift_to_EENADU', 'Start_Circulating', 'Agency', 'quantity', 'age',
            'customer_type', 'occupation'
        ]
        forms_data = request.env['customer.form'].sudo().search_read(
            domain, fields=required_fields, order=order_by, limit=limit, offset=offset
        )

        # Post-process the data (minimal: only date str and face_base64 prefix)
        result = []
        ...
        _logger.info(
            f"[CACHE] Data cached for filtered query key: {cache_key} (fetched {len(result)} of {total_count})")
