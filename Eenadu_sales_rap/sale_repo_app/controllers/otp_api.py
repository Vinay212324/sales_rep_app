...
class OtpAPI(http.Controller):
    ...
    # @http.route('/api/send_URL', auth='public', methods=['POST'], type='json', csrf=False)
    # def send_otp(self, **post):
    #     token = post.get('token')
    #     if not token:
    #         return {'success': False, 'message': 'Token is required', 'code': 403}
    ...
    #     if not phone:
    #         return {'status': 'error', 'message': 'Phone number is required'}
    #     var = "okokokokokokok happy"
    #     otp = str(random.randint(100000, 999999))
    #     msg = f"Dear {var},Thanks for placing an order. Your advertisement will be published as scheduled.Regards EENADU"
    #
    #     url = "https://www.smsstriker.com/API/sms.php"
    #     payload = {
    #         'username': 'EERETA',
